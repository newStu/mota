...
function getMapIcon(name: string) {
  return getIconUrl("icon/map", name);
}

export const MapTilesImage: { [key in MapTiles]: string } = {
  [MapTiles.WALL]: getMapIcon("wall"),
  [MapTiles.WALL_1]: getMapIcon("wall_1"),
  [MapTiles.MAGMA]: getMapIcon("magma"),
  [MapTiles.STAR]: getMapIcon("star"),
  [MapTiles.ROAD]: getMapIcon("road"),
  [MapTiles.DOOR_BLUE]: getMapIcon("door_blue"),
  [MapTiles.DOOR_RED]: getMapIcon("door_red"),
  [MapTiles.DOOR_YELLOW]: getMapIcon("door_yellow"),
